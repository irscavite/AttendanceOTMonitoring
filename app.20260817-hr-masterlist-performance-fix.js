...
const SESSION_KEY = "otPersistentSessionV4";
const LEGACY_SESSION_KEY = "hrOvertimeSessionV3";
...
async function logout(){
  ...
  // Stop this signed-out account from receiving device push on this browser.
  try{await window.OTPush?.detachCurrentUser?.();}catch{}
...
function showApp(){
  ...
  bindInternalChat();
  renderInternalChat();
  window.OTPush?.syncForSignedInUser?.().catch?.(()=>{});
  openPushTargetFromUrl();
}

function openPushTargetFromUrl(){
  try{
    const url=new URL(window.location.href);
    const target=url.searchParams.get("open");
    if(!target)return;
...
document.getElementById("pushNotificationBtn")?.addEventListener("click",async()=>{
  try{
    const result=await window.OTPush?.enableForCurrentUser?.();
    if(result?.enabled)showToast("Phone notifications enabled on this device.");
    else if(result?.reason)showToast(result.reason);
  }catch(error){showToast(error?.message||"Unable to enable phone notifications.");}
});
...
(async function restoreSession(){
  await initializeFirebaseLoginUI();
  // Firebase Auth is the source of truth. The local cache is only a profile hint.
  // Closing Chrome no longer logs the account out; explicit Logout does.
  let saved=null;
  try{
    const legacy=sessionStorage.getItem(LEGACY_SESSION_KEY)||localStorage.getItem(LEGACY_SESSION_KEY);
    const cached=localStorage.getItem(SESSION_KEY)||legacy;
  ...
  try{
    const fresh=await window.OTFirebase?.restoreSession?.();
    if(!fresh){
      localStorage.removeItem(SESSION_KEY);
      return;
    }
    ...
    console.error("Persistent session restore failed:",error);
    localStorage.removeItem(SESSION_KEY);
  }
})();
