...
const OT_AGREEMENT_KEY = "hrOTAgreementsV1";
...
const TAB_SCOPED_RECORD_KEYS = new Set([
  STORE_KEY,ATTENDANCE_STORE_KEY,EMPLOYEE_ATTENDANCE_OT_KEY,OT_AGREEMENT_KEY,LEAVE_REQUEST_KEY,
...
const AUDITED_STORAGE_MODULES = new Map([
  ...
  [OT_AGREEMENT_KEY,"Overtime Agreement"],
...
function newlyCompletedMilestones(before={},after={}){
  const oldCompletions=before?.milestoneCompletions||{};
  const newCompletions=after?.milestoneCompletions||{};
  return Object.entries(newCompletions).filter(([key,value])=>value && !oldCompletions?.[key]);
}
...
function auditStorageMutation(key,beforeRaw,afterRaw){
  ...
  entries.slice(0,maxDetailed).forEach(change=>{
    ...
    const milestoneChanges=key===EMPLOYEE_PRIVATE_PROFILE_KEY && change.after
      ? newlyCompletedMilestones(change.before||{},change.after)
      : [];
    if(milestoneChanges.length){
      milestoneChanges.forEach(([milestoneKey,completion])=>{
        const definition=EMPLOYEE_MILESTONE_DEFINITIONS.find(item=>item.key===milestoneKey);
        const milestoneLabel=definition?.label||"Employee Milestone";
        const milestoneDate=typeof completion==="object"?String(completion?.milestoneDate||""):"";
        appendSystemActivity({
          module:"Employee Milestone",
          action:"Completed",
          target:`${String(row.name||change.id||"Employee")} — ${milestoneLabel}`,
          detail:milestoneDate?`Scheduled milestone: ${formatDate(milestoneDate)}`:"Evaluation marked OK / Done",
          recordId:`${change.id}:${milestoneKey}`
        });
      });
      return;
    }
...
let supervisorEmployeeSubmissionDateKey = toDateKey(new Date());
let employeeAttendanceDateKey = toDateKey(new Date());
...
// ===== OVERTIME AGREEMENTS =====
// A Supervisor sends a Day Off / Holiday work notice. The employee answers
// OK or NO (with a required reason), and that response becomes a Supervisor
// notification. Agreements are kept separate from actual attendance/OT filings.
function getOTAgreements(){
  try{return JSON.parse(cloudGetItem(OT_AGREEMENT_KEY)) || []}
  catch{return []}
}
function saveOTAgreements(data){
  return cloudSetItem(OT_AGREEMENT_KEY,JSON.stringify(data));
}
function dateKeyOffset(dateKey,days){
  const base=/^\d{4}-\d{2}-\d{2}$/.test(String(dateKey||""))?parseLocalDate(dateKey):new Date();
  base.setDate(base.getDate()+Number(days||0));
  return toDateKey(base);
}
function employeeOTAgreements(employeeNo){
  return getOTAgreements().filter(row=>String(row.employeeNo||"")===String(employeeNo||""));
}
function departmentOTAgreements(department){
  return getOTAgreements().filter(row=>normalizeDepartmentName(row.department)===normalizeDepartmentName(department));
}
function pendingEmployeeOTAgreementCount(employeeNo){
  return employeeOTAgreements(employeeNo).filter(row=>row.status==="Pending Response").length;
}
function unreadEmployeeOTAgreementCount(employeeNo){
  return employeeOTAgreements(employeeNo).filter(row=>!row.employeeNoticeRead && row.status!=="Withdrawn").length;
}
function unreadSupervisorOTAgreementResponseCount(department){
  return departmentOTAgreements(department).filter(row=>["Accepted","Declined"].includes(row.status) && !row.supervisorResponseRead).length;
}
function markEmployeeOTAgreementRead(id){
  const rows=getOTAgreements();
  const item=rows.find(row=>String(row.id)===String(id) && String(row.employeeNo)===String(currentUser?.employeeNo));
  if(item){item.employeeNoticeRead=true;item.employeeNoticeReadAt=new Date().toISOString();saveOTAgreements(rows);updateEmployeeLeaveNotificationUI?.();}
}
function markAllEmployeeOTAgreementsRead(){
  const employeeNo=String(currentUser?.employeeNo||"");
  const now=new Date().toISOString();
  saveOTAgreements(getOTAgreements().map(row=>String(row.employeeNo)===employeeNo?{...row,employeeNoticeRead:true,employeeNoticeReadAt:row.employeeNoticeReadAt||now}:row));
}
function markSupervisorOTAgreementResponseRead(id){
  const rows=getOTAgreements();
  const item=rows.find(row=>String(row.id)===String(id) && normalizeDepartmentName(row.department)===normalizeDepartmentName(currentUser?.department));
  if(item){item.supervisorResponseRead=true;item.supervisorResponseReadAt=new Date().toISOString();saveOTAgreements(rows);updateSupervisorEmployeeNotificationUI?.();}
}
function markAllSupervisorOTAgreementResponsesRead(){
  const department=currentUser?.department;
  const now=new Date().toISOString();
  saveOTAgreements(getOTAgreements().map(row=>normalizeDepartmentName(row.department)===normalizeDepartmentName(department) && ["Accepted","Declined"].includes(row.status)?{...row,supervisorResponseRead:true,supervisorResponseReadAt:row.supervisorResponseReadAt||now}:row));
}
async function respondToOTAgreement(id,response,reason=""){
  const normalized=response==="Accepted"?"Accepted":"Declined";
  const responseReason=String(reason||"").trim();
  if(normalized==="Declined" && !responseReason)throw new Error("Reason is required when you answer NO.");
  const rows=getOTAgreements();
  const item=rows.find(row=>String(row.id)===String(id) && String(row.employeeNo)===String(currentUser?.employeeNo));
  if(!item)throw new Error("Overtime agreement could not be found.");
  if(item.status!=="Pending Response")throw new Error("This overtime agreement already has a response.");
  const now=new Date().toISOString();
  Object.assign(item,{status:normalized,responseReason:normalized==="Declined"?responseReason:"",respondedAt:now,respondedBy:currentUser.displayName||currentUser.username,employeeNoticeRead:true,employeeNoticeReadAt:now,supervisorResponseRead:false,supervisorResponseReadAt:"",updatedAt:now});
  const saved=await saveOTAgreements(rows);
  if(saved===false)throw new Error("Your response is kept on this device, but Firebase could not save it. Check the connection or publish the included database rules, then try again.");
  return item;
}
...
function addSupervisorLeaveSubmissionNotification(request){
  ...
  // Employee accounts cannot write the shared Supervisor notification store.
  // Their leave request is already synced through the department-scoped leave
  // collection, which updates the Supervisor queue and navigation badge live.
...
function updateEmployeeLeaveNotificationUI(){
  ...
  const employeeNo=currentUser.employeeNo;
  const leaveNotifications=employeeLeaveNotificationsFor(employeeNo).map(n=>({...n,notificationKind:"leave"}));
  const agreementNotifications=employeeOTAgreements(employeeNo)
    .filter(row=>row.status!=="Withdrawn")
    .map(row=>({
      id:row.id,notificationKind:"ot-agreement",read:Boolean(row.employeeNoticeRead),
      title:row.status==="Pending Response"?"Overtime agreement needs your response":`Overtime agreement: ${row.status}`,
      message:`${formatDate(row.dutyDate)} • ${row.otType||"Day Off / Holiday OT"}`,
      details:row.status==="Pending Response"?`${row.supervisorName||"Your Supervisor"} asks you to confirm OK or NO. If you answer OK, file Advance OT or Attendance & OT for this date.${row.message?` ${row.message}`:""}`:(row.responseReason?`Your reason: ${row.responseReason}`:"Response recorded."),
      targetPage:"ot-agreements",createdAt:row.updatedAt||row.createdAt
    }));
  const notifications=leaveNotifications.concat(agreementNotifications).sort((a,b)=>new Date(b.createdAt||0)-new Date(a.createdAt||0));
  const unread=notifications.filter(n=>!n.read).length;
  ...
  if(list){
    list.innerHTML=notifications.length ? notifications.slice(0,15).map(n=>{
      const isAgreement=n.notificationKind==="ot-agreement";
      const approved=String(n.type||"").includes("approved");
      const rejected=String(n.type||"").includes("rejected");
      const icon=isAgreement?"OT":rejected?"×":approved?"✓":"L";
      return `
      <button class="notification-item employee-leave-alert ${n.read?"":"unread"}" type="button" data-employee-notification-id="${escapeHtml(n.id)}" data-notification-kind="${escapeHtml(n.notificationKind||"leave")}" data-target-page="${escapeHtml(n.targetPage||"my-leave")}">
        <span class="notification-item-icon">${icon}</span>
        <span class="notification-item-copy">
          <strong>${escapeHtml(n.title)}</strong>
          <span>${escapeHtml(n.message)}</span>
    ...
    list.querySelectorAll(".notification-item").forEach(btn=>{
      btn.addEventListener("click",()=>{
        if(btn.dataset.notificationKind==="ot-agreement")markEmployeeOTAgreementRead(btn.dataset.employeeNotificationId);
        else markEmployeeLeaveNotificationRead(btn.dataset.employeeNotificationId);
...
function updateSupervisorEmployeeNotificationUI(){
  ...
  const workflowNotifications=supervisorNotificationsForDepartment(currentUser.department).map(n=>({...n,notificationKind:"workflow"}));
  const agreementNotifications=departmentOTAgreements(currentUser.department)
    .filter(row=>["Accepted","Declined"].includes(row.status))
    .map(row=>({
      id:row.id,notificationKind:"ot-agreement-response",read:Boolean(row.supervisorResponseRead),type:row.status==="Accepted"?"ot-agreement-accepted":"ot-agreement-declined",
      title:`${row.employeeName||"Employee"} answered ${row.status==="Accepted"?"OK":"NO"}`,
      message:`${formatDate(row.dutyDate)} • ${row.otType||"Day Off / Holiday OT"}`,
      details:row.status==="Declined"?`Reason: ${row.responseReason||"No reason recorded"}`:"Employee confirmed availability and can proceed with OT filing.",
      targetPage:"ot-agreements",createdAt:row.respondedAt||row.updatedAt||row.createdAt
    }));
  const notifications=workflowNotifications.concat(agreementNotifications).sort((a,b)=>new Date(b.createdAt||0)-new Date(a.createdAt||0));
  const unread=notifications.filter(n=>!n.read).length;
  ...
  if(list){
    list.innerHTML=notifications.length ? notifications.slice(0,15).map(n=>{
      ...
      const isAgreementResponse=n.notificationKind==="ot-agreement-response";
      const icon=isAgreementResponse?(n.type==="ot-agreement-accepted"?"OK":"NO"):n.type==="ot-final-approved"?"✓":isPendingLeave?"L":isFinalLeave?(n.type==="leave-final-approved"?"✓":"×"):"◷";
      return `
      <button class="notification-item supervisor-employee-alert ${n.read?"":"unread"}" type="button" data-supervisor-notification-id="${escapeHtml(n.id)}" data-notification-kind="${escapeHtml(n.notificationKind||"workflow")}" data-target-page="${escapeHtml(n.targetPage || "employee-schedule")}">
        <span class="notification-item-icon">${icon}</span>
        <span class="notification-item-copy">
          <strong>${escapeHtml(n.title)}</strong>
          <span>${escapeHtml(n.message)}</span>
    ...
    list.querySelectorAll(".notification-item").forEach(btn=>{
      btn.addEventListener("click",()=>{
        if(btn.dataset.notificationKind==="ot-agreement-response")markSupervisorOTAgreementResponseRead(btn.dataset.supervisorNotificationId);
        else markSupervisorEmployeeNotificationRead(btn.dataset.supervisorNotificationId);
...
function bindSupervisorEmployeeNotificationControls(){
  ...
  document.getElementById("markAllSupervisorNotificationsRead")?.addEventListener("click",e=>{
    ...
    markAllSupervisorOTAgreementResponsesRead();
  });
...
function getSharedAdminRecentActivityItems(limit=8){
  ...
  // Milestone completions are stored with the employee's private profile.
  // Build the activity item from that Firebase-backed record as well, so an
  // already-completed evaluation remains visible even after a refresh.
  const milestoneCompletionItems=activeEmployees.flatMap(employee=>{
    const profile=privateProfiles?.[employee.no]||{};
    const completions=profile.milestoneCompletions||{};
    return EMPLOYEE_MILESTONE_DEFINITIONS.flatMap(definition=>{
      const completion=completions?.[definition.key];
      if(!completion)return [];
      const completionData=typeof completion==="object"?completion:{};
      const milestoneDate=String(completionData.milestoneDate||"").trim();
      const at=String(completionData.completedAt||"").trim() || (milestoneDate?`${milestoneDate}T12:00:00`:"");
      if(!at)return [];
      const eventTime=new Date(at).getTime();
      const recordId=`${employee.no}:${definition.key}`;
      return [{
        kind:"milestone",action:"Completed",target:`${employee.name||employee.no||"Employee"} — ${definition.label}`,module:"Employee Milestone",
        detail:milestoneDate?`Scheduled milestone: ${formatDate(milestoneDate)}`:"Evaluation marked OK / Done",
        actorName:completionData.completedBy||"HR / Admin Supervisor",at,dateKey:String(at).slice(0,10),
        sortTime:Number.isFinite(eventTime)?eventTime:0,recordId,sourceId:`milestone-completion:${recordId}:${at}`
      }];
    });
  });
  ...
  const milestoneCompletionIds=new Set(milestoneCompletionItems.map(item=>String(item.recordId||"")).filter(Boolean));
  const visibleLoggedItems=loggedItems.filter(item=>{
    if(item.module==="Employee Milestone" && item.recordId && milestoneCompletionIds.has(String(item.recordId)))return false;
    if(item.module==="Employee Profile" && item.recordId && !String(item.detail||"").trim()){
      const isLegacyMilestoneDuplicate=milestoneCompletionItems.some(milestone=>
        String(milestone.recordId||"").startsWith(`${String(item.recordId)}:`) &&
        Math.abs(Number(milestone.sortTime||0)-Number(item.sortTime||0))<=60000
      );
      if(isLegacyMilestoneDuplicate)return false;
    }
  ...
  const seen=new Set();
  return [...visibleLoggedItems,...milestoneCompletionItems,...recruitmentHireItems,...applicantHireItems,...hiredItems,...approvedOtItems]
...
function renderSharedAdminRecentActivity(items=[]){
  ...
  return `<div class="ems-recent-list">${rows.map(item=>{
    const icon=item.kind==="hire"?emsDashboardIcon("userPlus"):item.kind==="ot"?emsDashboardIcon("clock"):item.kind==="milestone"?emsDashboardIcon("check"):emsDashboardIcon("clipboard");
...
async function logout(){
  ...
  currentUser=null;
  appView.classList.remove("employee-mobile-app","approver-mobile-app","responsive-mobile-app","supervisor-app");
  appView.classList.add("hidden");
  loginView.classList.remove("hidden");
  loginForm.reset();
}

// A narrow window (or a touch-first phone/tablet in landscape) gets the mobile
// shell regardless of account role. The same account keeps its desktop shell on
// wider laptop/desktop windows; no database or authentication changes are needed.
const responsiveMobileQuery=window.matchMedia("(max-width: 790px), (pointer: coarse) and (max-width: 1024px)");
function syncResponsiveAppLayout(){
  const mobile=Boolean(currentUser && responsiveMobileQuery.matches);
  const mobileAccount=mobile && ["Employee","Request Approver"].includes(currentUser.role);
  appView.classList.toggle("responsive-mobile-app",mobile);
  appView.classList.toggle("employee-mobile-app",mobileAccount);
  appView.classList.toggle("approver-mobile-app",mobile && currentUser?.role==="Request Approver");
  document.getElementById("employeeMobileLogoutBtn")?.classList.toggle("hidden",!mobile);
}
responsiveMobileQuery.addEventListener?.("change",syncResponsiveAppLayout);
// Resize also handles browsers that do not dispatch MediaQueryList change events.
window.addEventListener("resize",syncResponsiveAppLayout,{passive:true});

function showApp(){
  loginView.classList.add("hidden");
  appView.classList.remove("hidden");
  appView.classList.toggle("supervisor-app",currentUser.role==="Supervisor");
  syncResponsiveAppLayout();
...
function buildNavigation(){
  ...
  const nav = {
    Supervisor:[
      ...
      {id:"new-request",label:"Attendance & OT",icon:"＋"},
      {id:"ot-agreements",label:"OT Agreements",icon:"✓"},
  ...
  sideNav.setAttribute("aria-label","Main navigation; swipe horizontally on mobile for more pages");
  sideNav.innerHTML = nav.map(item=>item.separator
    ...
    <button class="nav-btn ${currentPage===item.id?"active":""}" data-page="${item.id}" type="button">
      ...
      ${currentUser.role==="Supervisor" && item.id==="ot-agreements" && unreadSupervisorOTAgreementResponseCount(currentUser.department) ? `<span class="nav-count-badge">${unreadSupervisorOTAgreementResponseCount(currentUser.department)}</span>` : ""}
      ${currentUser.role==="Employee" && item.id==="ot-agreements" && pendingEmployeeOTAgreementCount(currentUser.employeeNo) ? `<span class="nav-count-badge">${pendingEmployeeOTAgreementCount(currentUser.employeeNo)}</span>` : ""}
...
function navigateTo(page){
  ...
  if(currentUser?.role==="Employee" && page==="attendance-ot")employeeAttendanceDateKey=toDateKey(new Date());
  if(currentUser?.role==="Supervisor" && page==="new-request")supervisorEmployeeSubmissionDateKey=toDateKey(new Date());
  currentPage=page;
...
function getOperationalDataSummary(){
  ...
  return {
    ...
    otAgreements:getOTAgreements().length,
...
function openITClearOperationalDataModal(){
  ...
  const transactionTotal=summary.overtime+summary.leave+summary.attendance+summary.employeeSubmissions+summary.otAgreements;
  modalRoot.innerHTML=`<div class="modal-backdrop" id="itClearOperationalBackdrop"><div class="modal" role="dialog" aria-modal="true"><div class="modal-header"><span>IT • DATA MAINTENANCE</span><h3>Clear Attendance, OT &amp; Leave Data?</h3></div><div class="modal-body"><div class="note-box" style="margin-bottom:14px"><strong>${transactionTotal} filed transaction(s) will be permanently cleared.</strong><br><small>This removes filed Attendance, Employee OT submissions, OT requests and decisions, OT agreements, Leave requests and decisions, plus their related workflow notifications.</small><br><small><strong>Preserved:</strong> Employee Master List, accounts, Work Area, Morning/Night Schedule, Shift In/Out, Day Off, SL/VL setup, IT access, account-creation records, and internal chat.</small></div><div class="master-attendance-history-summary"><div><span>Attendance Entries</span><strong>${summary.attendance+summary.employeeSubmissions}</strong></div><div><span>OT Requests / Agreements</span><strong>${summary.overtime+summary.otAgreements}</strong></div><div><span>Leave Requests</span><strong>${summary.leave}</strong></div></div><div class="note-box" style="margin-top:14px"><small>Approved Leave records will be removed, so used/reserved leave counts return to zero.</small></div><label class="field" style="margin-top:14px"><span>Type CLEAR FILED DATA to confirm</span><input id="itClearOperationalConfirmText" autocomplete="off" placeholder="CLEAR FILED DATA"></label></div><div class="modal-footer"><button id="itCancelClearOperational" class="btn btn-light" type="button">Cancel</button><button id="itConfirmClearOperational" class="btn btn-danger" type="button" disabled>Clear Filed Attendance Data</button></div></div></div>`;
  const close=()=>{modalRoot.innerHTML="";};
...
function getSupervisorForDepartment(department){
  ...
  const match=getSystemAccountDirectory().find(user=>
    user?.role==="Supervisor" &&
    normalizeDepartmentName(user.department||user.employeeDepartment)===targetDepartment &&
    user.active!==false
  );
...
function renderEmployeeAttendanceOT(employeeBase){
  ...
  const dateKey=/^\d{4}-\d{2}-\d{2}$/.test(candidateDate) ? candidateDate : todayKey;
  ...
  const linkedAgreement=employeeOTAgreements(employee.no).find(row=>row.dutyDate===dateKey && row.status!=="Withdrawn") || null;
  ...
  const defaultOtType=holiday?getAutomaticOtType(dateKey,false):(prior?.otType || linkedAgreement?.otType || (isDayOff ? "Rest Day OT" : "Regular Day"));
  const fullDutyDefault=Boolean(holiday) || (isDayOff && defaultOtType==="Rest Day OT");
  const defaultOtStart=prior?.normalOtStart || linkedAgreement?.otStart || (fullDutyDefault?schedule.start:schedule.end);
  ...
  setPage(pageTitle,pageEyebrow,`Attendance/OT date: ${formatDate(dateKey)} • the default is today's actual calendar date.`);
  ...
  const priorAttendance=(isAdvanceFiling || linkedAgreement?.status==="Accepted")?"Present":defaultStatus;
  const hasOT=Boolean(isAdvanceFiling || linkedAgreement?.status==="Accepted" || (prior?.hasOT && priorAttendance==="Present"));
  ...
  content.innerHTML=`
    ...
    ${linkedAgreement?`<div class="info-banner ot-agreement-linked-banner"><span class="info-icon">OT</span><div><strong>Supervisor OT Agreement — ${escapeHtml(linkedAgreement.status)}</strong><br><small>${escapeHtml(linkedAgreement.requirementType||"Render Overtime")} • Expected ${escapeHtml(linkedAgreement.otStart||"—")} → ${escapeHtml(linkedAgreement.otEnd||"—")} • ${escapeHtml(linkedAgreement.message||"")}</small></div></div>`:""}
      ...
      <div class="card-body">
        <form id="employeeAttendanceOTForm" class="employee-attendance-ot-form">
          <div class="employee-attendance-toolbar">
            <label class="field compact-field"><span>Attendance / OT Date</span><input id="employeeAttendanceDate" type="date" value="${escapeHtml(dateKey)}" required><small>Select a future date to file advance OT.</small></label>
          ...
          <div class="employee-attendance-section employee-ot-section">
            ...
            <div id="employeeOTFields" class="employee-ot-fields ${hasOT?"":"hidden"}">
              <div class="employee-ot-grid">
                ...
                <label class="field"><span>OT Out</span><input id="employeeOTEnd" class="time-24-input" type="text" inputmode="numeric" maxlength="5" placeholder="HH:MM" value="${escapeHtml(prior?.normalOtEnd||linkedAgreement?.otEnd||"")}" ${otReviewLocked?"disabled":""}></label>
  ...
  dateInput?.addEventListener("change",()=>{
    ...
    if(!/^\d{4}-\d{2}-\d{2}$/.test(selected)){
      employeeAttendanceDateKey=todayKey;
  ...
  document.getElementById("employeeAttendanceOTForm")?.addEventListener("submit",async e=>{
    ...
    if(employeeHasOT && linkedAgreement?.status==="Accepted"){
      const agreements=getOTAgreements();
      const agreement=agreements.find(row=>String(row.id)===String(linkedAgreement.id));
      if(agreement){agreement.filingStatus="OT Filed";agreement.otFiledAt=new Date().toISOString();agreement.otSubmissionId=saved.id;agreement.updatedAt=new Date().toISOString();saveOTAgreements(agreements);}
    }
...
function otAgreementStatusBadge(status){
  if(status==="Accepted")return "approved";
  if(status==="Declined" || status==="Withdrawn")return "rejected";
  return "warning";
}
function eligibleForOTAgreement(employee,dutyDate){
  const holiday=getPhilippineHoliday(dutyDate);
  const weekday=parseLocalDate(dutyDate).toLocaleDateString("en-PH",{weekday:"long"});
  return Boolean(holiday || (employee.dayOff && employee.dayOff===weekday));
}
function renderSupervisorOTAgreements(){
  const employees=getDepartmentEmployees(currentUser.department).filter(isEmployeeSetupComplete).filter(emp=>String(emp.no)!==String(currentUser.employeeNo||""));
  const todayKey=toDateKey(new Date());
  const defaultDate=dateKeyOffset(todayKey,1);
  const rows=departmentOTAgreements(currentUser.department).sort((a,b)=>String(b.dutyDate||"").localeCompare(String(a.dutyDate||"")) || new Date(b.updatedAt||b.createdAt||0)-new Date(a.updatedAt||a.createdAt||0));
  const pending=rows.filter(row=>row.status==="Pending Response").length;
  const accepted=rows.filter(row=>row.status==="Accepted").length;
  const declined=rows.filter(row=>row.status==="Declined").length;
  setPage("OT Agreements","SUPERVISOR NOTIFICATION","Notify members who need to report for duty or render overtime on their Day Off or a Philippine Holiday.");
  content.innerHTML=`${heroBanner("Day Off / Holiday overtime agreement","Send the notice in advance. The member answers OK or NO; a reason is mandatory for NO.",rows.length,"agreement notice(s)")}
    <div class="stats-grid">${statCard("Waiting",pending,"Employee response pending","warning")}${statCard("OK",accepted,"Members confirmed","success")}${statCard("NO",declined,"Members unavailable","danger")}${statCard("Unread Replies",unreadSupervisorOTAgreementResponseCount(currentUser.department),"New answers for Supervisor","primary")}</div>
    <div class="grid-2 ot-agreement-layout">
      <div class="card"><div class="card-header"><div class="card-title-group"><h3>Notify Member</h3><p>Only a scheduled Day Off or configured Philippine Holiday can be selected.</p></div><span class="badge pending">Advance Notice</span></div><div class="card-body">
        <form id="supervisorOTAgreementForm" class="ot-agreement-form">
          <label class="field"><span>Duty / OT Date</span><input id="otAgreementDutyDate" type="date" min="${todayKey}" value="${defaultDate}" required></label>
          <label class="field"><span>Member</span><select id="otAgreementEmployee" required><option value="ALL_ELIGIBLE">All eligible members for selected date</option>${employees.map(emp=>`<option value="${escapeHtml(emp.no)}">${escapeHtml(emp.name)} — ${escapeHtml(emp.dayOff||"No Day Off set")}</option>`).join("")}</select></label>
          <label class="field"><span>Requirement</span><select id="otAgreementRequirement"><option value="Report for Duty">Report for Duty</option><option value="Render Overtime">Render Overtime</option></select></label>
          <label class="employee-straight-duty-toggle"><input id="otAgreementUseSchedule" type="checkbox" checked><span>Use each member's assigned Shift In / Out</span></label>
          <div class="revision-time-grid"><label class="field"><span>Expected OT In</span><input id="otAgreementStart" class="time-24-input" type="text" inputmode="numeric" maxlength="5" value="06:00" placeholder="HH:MM" required></label><label class="field"><span>Expected OT Out</span><input id="otAgreementEnd" class="time-24-input" type="text" inputmode="numeric" maxlength="5" value="15:00" placeholder="HH:MM" required></label></div>
          <label class="field"><span>Instruction / Reason</span><textarea id="otAgreementMessage" rows="3" required placeholder="Example: Required yard operation / urgent container movement"></textarea></label>
          <div id="otAgreementEligibilityNote" class="schedule-management-note">Select the date to verify eligible Day Off / Holiday members.</div>
          <button class="btn btn-primary" type="submit">Send OT Agreement Notification</button>
        </form>
      </div></div>
      <div class="card ot-agreement-history-card"><div class="card-header"><div class="card-title-group"><h3>Member Responses</h3><p>OK / NO responses are recorded with date and time.</p></div></div><div class="card-body">
        <div class="ot-agreement-list">${rows.length?rows.map(row=>`<article class="ot-agreement-card ${String(row.status||"").toLowerCase().replace(/\s+/g,"-")}"><div class="ot-agreement-card-head"><div><strong>${escapeHtml(row.employeeName)}</strong><span>${escapeHtml(row.position||"—")} • ${escapeHtml(row.workArea||"—")}</span></div><span class="badge ${otAgreementStatusBadge(row.status)}">${escapeHtml(row.status||"Pending Response")}</span></div><div class="ot-agreement-details"><div><span>Date</span><strong>${formatDate(row.dutyDate)}</strong></div><div><span>OT Type</span><strong>${escapeHtml(row.otType||"—")}</strong></div><div><span>Expected Time</span><strong>${escapeHtml(row.otStart||"—")} → ${escapeHtml(row.otEnd||"—")}</strong></div><div><span>Expected OT</span><strong>${Number(row.expectedHours||0).toFixed(2)} h</strong></div></div><p>${escapeHtml(row.message||"")}</p>${row.status==="Declined"?`<div class="ot-agreement-response decline"><strong>NO — Reason:</strong> ${escapeHtml(row.responseReason||"No reason recorded")}</div>`:row.status==="Accepted"?`<div class="ot-agreement-response accept"><strong>OK</strong> — Confirmed ${formatDateTime(row.respondedAt)}</div>`:`<div class="ot-agreement-response pending">Waiting for member response</div>`}<div class="actions">${["Accepted","Declined"].includes(row.status)&&!row.supervisorResponseRead?`<button class="btn btn-light btn-sm" type="button" data-agreement-seen="${escapeHtml(row.id)}">Mark Response Seen</button>`:""}${row.status==="Pending Response"?`<button class="btn btn-danger btn-sm" type="button" data-agreement-withdraw="${escapeHtml(row.id)}">Withdraw Notice</button>`:""}</div></article>`).join(""):emptyState("No OT agreements yet","Use the form to notify eligible members about Day Off or Holiday duty.")}</div>
      </div></div>
    </div>`;
  const dateInput=document.getElementById("otAgreementDutyDate");
  const employeeInput=document.getElementById("otAgreementEmployee");
  const useScheduleInput=document.getElementById("otAgreementUseSchedule");
  const startInput=document.getElementById("otAgreementStart");
  const endInput=document.getElementById("otAgreementEnd");
  const updateTimeMode=()=>{const assigned=Boolean(useScheduleInput?.checked);if(startInput)startInput.disabled=assigned;if(endInput)endInput.disabled=assigned;};
  useScheduleInput?.addEventListener("change",updateTimeMode);updateTimeMode();
  const updateEligibility=()=>{
    const dateKey=dateInput?.value||defaultDate;
    const holiday=getPhilippineHoliday(dateKey);
    const eligible=employees.filter(emp=>eligibleForOTAgreement(emp,dateKey));
    const note=document.getElementById("otAgreementEligibilityNote");
    if(note)note.innerHTML=holiday?`<strong>${escapeHtml(holiday.name)}</strong> • All ${eligible.length} department member(s) are eligible for a Holiday OT notice.`:`${eligible.length} member(s) have their scheduled Day Off on ${escapeHtml(parseLocalDate(dateKey).toLocaleDateString("en-PH",{weekday:"long"}))}.`;
  };
  dateInput?.addEventListener("change",updateEligibility);updateEligibility();
  document.getElementById("supervisorOTAgreementForm")?.addEventListener("submit",async event=>{
    event.preventDefault();
    const dutyDate=dateInput?.value||"";
    const selected=employeeInput?.value||"";
    const requirementType=document.getElementById("otAgreementRequirement")?.value||"Report for Duty";
    const useAssignedSchedule=Boolean(useScheduleInput?.checked);
    const customOtStart=normalize24HourTime(startInput?.value||"");
    const customOtEnd=normalize24HourTime(endInput?.value||"");
    const message=document.getElementById("otAgreementMessage")?.value.trim()||"";
    if(!dutyDate || dutyDate<todayKey){showToast("Select today or a future Day Off / Holiday date.");return;}
    if(!useAssignedSchedule && (!isValid24HourTime(customOtStart)||!isValid24HourTime(customOtEnd))){showToast("Enter valid Expected OT In and OT Out in HH:MM format.");return;}
    if(!message){showToast("Enter the instruction or reason for the overtime notice.");return;}
    let targets=selected==="ALL_ELIGIBLE"?employees.filter(emp=>eligibleForOTAgreement(emp,dutyDate)):employees.filter(emp=>String(emp.no)===String(selected));
    if(!targets.length){showToast("No eligible member was found for this date.");return;}
    const ineligible=targets.filter(emp=>!eligibleForOTAgreement(emp,dutyDate));
    if(ineligible.length){showToast(`${ineligible[0].name} is not scheduled for Day Off and this date is not a configured Philippine Holiday.`);return;}
    const holiday=getPhilippineHoliday(dutyDate);
    const invalidHoursEmployee=targets.find(emp=>{const schedule=getEmployeeAttendanceSchedule(emp);const otType=holiday?getAutomaticOtType(dutyDate,false):"Rest Day OT";const normalOtStart=useAssignedSchedule?schedule.start:customOtStart;const normalOtEnd=useAssignedSchedule?schedule.end:customOtEnd;return Number(proposedOTBreakdown({scheduleStart:schedule.start,scheduleEnd:schedule.end,normalOtStart,normalOtEnd,otType,otDate:dutyDate}).total||0)<=0;});
    if(invalidHoursEmployee){showToast(`Expected OT for ${invalidHoursEmployee.name} must reach at least 1 hour.`);return;}
    const all=getOTAgreements();
    const now=new Date().toISOString();
    targets.forEach(emp=>{
      const schedule=getEmployeeAttendanceSchedule(emp);
      const otType=holiday?getAutomaticOtType(dutyDate,false):"Rest Day OT";
      const otStart=useAssignedSchedule?schedule.start:customOtStart;
      const otEnd=useAssignedSchedule?schedule.end:customOtEnd;
      const breakdown=proposedOTBreakdown({scheduleStart:schedule.start,scheduleEnd:schedule.end,normalOtStart:otStart,normalOtEnd:otEnd,otType,otDate:dutyDate});
      const existing=all.find(row=>String(row.employeeNo)===String(emp.no)&&row.dutyDate===dutyDate&&row.status!=="Withdrawn");
      const payload={id:existing?.id||`AGR-${Date.now()}-${Math.floor(Math.random()*100000)}`,employeeNo:emp.no,employeeName:emp.name,position:emp.position||"",department:emp.department||currentUser.department,workArea:emp.location||"",dutyDate,requirementType,otType,otStart,otEnd,expectedHours:Number(breakdown.total||0).toFixed(2),holidayName:holiday?.name||"",message,supervisorName:currentUser.displayName||currentUser.username,supervisorUsername:currentUser.username,status:"Pending Response",responseReason:"",respondedAt:"",respondedBy:"",employeeNoticeRead:false,employeeNoticeReadAt:"",supervisorResponseRead:true,supervisorResponseReadAt:"",createdAt:existing?.createdAt||now,updatedAt:now};
      if(existing)Object.assign(existing,payload);else all.unshift(payload);
    });
    const saved=await saveOTAgreements(all);
    if(saved===false){showToast("Agreement was kept on this device, but Firebase could not save it. Check the connection or publish the included database rules.");return;}
    showToast(`${targets.length} member${targets.length===1?"":"s"} notified. Waiting for OK or NO response.`);buildNavigation();renderSupervisorOTAgreements();
  });
  document.querySelectorAll("[data-agreement-seen]").forEach(button=>button.addEventListener("click",()=>{markSupervisorOTAgreementResponseRead(button.dataset.agreementSeen);buildNavigation();renderSupervisorOTAgreements();}));
  document.querySelectorAll("[data-agreement-withdraw]").forEach(button=>button.addEventListener("click",async()=>{const all=getOTAgreements();const item=all.find(row=>String(row.id)===String(button.dataset.agreementWithdraw));if(!item)return;Object.assign(item,{status:"Withdrawn",updatedAt:new Date().toISOString(),employeeNoticeRead:false});await saveOTAgreements(all);showToast("OT agreement notice withdrawn.");buildNavigation();renderSupervisorOTAgreements();}));
}

function openEmployeeOTAgreementDeclineModal(id){
  const item=employeeOTAgreements(currentUser.employeeNo).find(row=>String(row.id)===String(id));
  if(!item)return;
  modalRoot.innerHTML=`<div class="modal-backdrop" id="otAgreementDeclineBackdrop"><div class="modal"><div class="modal-header"><span>OT AGREEMENT RESPONSE</span><h3>Answer NO</h3></div><div class="modal-body"><div class="note-box"><strong>${formatDate(item.dutyDate)}</strong> • ${escapeHtml(item.otType||"Overtime")}<br>${escapeHtml(item.otStart||"—")} → ${escapeHtml(item.otEnd||"—")}</div><label class="field" style="margin-top:14px"><span>Reason (Required)</span><textarea id="otAgreementDeclineReason" rows="4" placeholder="Explain why you cannot report for duty or render overtime"></textarea></label></div><div class="modal-footer"><button id="cancelOTAgreementDecline" class="btn btn-light" type="button">Cancel</button><button id="confirmOTAgreementDecline" class="btn btn-danger" type="button">Submit NO Response</button></div></div></div>`;
  const close=()=>modalRoot.innerHTML="";
  document.getElementById("cancelOTAgreementDecline")?.addEventListener("click",close);
  document.getElementById("otAgreementDeclineBackdrop")?.addEventListener("click",event=>{if(event.target.id==="otAgreementDeclineBackdrop")close();});
  document.getElementById("confirmOTAgreementDecline")?.addEventListener("click",async()=>{const reason=document.getElementById("otAgreementDeclineReason")?.value.trim()||"";if(!reason){showToast("Reason is required when you answer NO.");return;}try{await respondToOTAgreement(id,"Declined",reason);close();showToast("NO response sent. Your Supervisor has been notified with your reason.");buildNavigation();renderEmployeeOTAgreements();}catch(error){showToast(error?.message||"Unable to save your response.");}});
}
function renderEmployeeOTAgreements(){
  const employee=getCurrentEmployeeRecord();
  if(!employee)return;
  const rows=employeeOTAgreements(employee.no).sort((a,b)=>String(b.dutyDate||"").localeCompare(String(a.dutyDate||"")) || new Date(b.updatedAt||0)-new Date(a.updatedAt||0));
  const pending=rows.filter(row=>row.status==="Pending Response").length;
  setPage("OT Agreements","EMPLOYEE RESPONSE","Review Day Off / Holiday work notices from your Supervisor and answer OK or NO.");
  content.innerHTML=`${heroBanner("Overtime agreement & availability","A NO answer requires a reason. Your answer immediately notifies your Supervisor.",pending,"response(s) needed")}
    <div class="ot-agreement-employee-list">${rows.length?rows.map(row=>`<article class="card ot-agreement-employee-card"><div class="card-header"><div class="card-title-group"><h3>${formatDate(row.dutyDate)} — ${escapeHtml(row.requirementType||"Render Overtime")}</h3><p>${escapeHtml(row.supervisorName||"Supervisor")} • ${escapeHtml(row.department||"")}</p></div><span class="badge ${otAgreementStatusBadge(row.status)}">${escapeHtml(row.status||"Pending Response")}</span></div><div class="card-body"><div class="ot-agreement-details"><div><span>OT Type</span><strong>${escapeHtml(row.otType||"—")}</strong></div><div><span>Expected Time</span><strong>${escapeHtml(row.otStart||"—")} → ${escapeHtml(row.otEnd||"—")}</strong></div><div><span>Expected OT</span><strong>${Number(row.expectedHours||0).toFixed(2)} h</strong></div><div><span>Work Area</span><strong>${escapeHtml(row.workArea||employee.location||"—")}</strong></div></div><div class="note-box"><strong>Supervisor instruction</strong><br>${escapeHtml(row.message||"Please confirm your availability.")}</div>${row.status==="Pending Response"?`<div class="ot-agreement-choice"><button class="btn btn-primary" type="button" data-agreement-ok="${escapeHtml(row.id)}">OK — I Confirm</button><button class="btn btn-danger" type="button" data-agreement-no="${escapeHtml(row.id)}">NO — Give Reason</button></div>`:row.status==="Declined"?`<div class="ot-agreement-response decline"><strong>Your NO reason:</strong> ${escapeHtml(row.responseReason||"")}</div>`:row.status==="Accepted"?`<div class="ot-agreement-response accept"><strong>OK confirmed</strong> • ${formatDateTime(row.respondedAt)}</div><button class="btn btn-primary file-agreement-ot" type="button" data-agreement-file="${escapeHtml(row.id)}">${row.dutyDate>toDateKey(new Date())?"File Advance OT":"File Attendance & OT"}</button>`:`<div class="ot-agreement-response decline">This notice was withdrawn by your Supervisor.</div>`}</div></article>`).join(""):emptyState("No overtime agreement notices","Day Off or Holiday duty notices from your Supervisor will appear here.")}</div>`;
  document.querySelectorAll("[data-agreement-ok]").forEach(button=>button.addEventListener("click",async()=>{try{await respondToOTAgreement(button.dataset.agreementOk,"Accepted");showToast("OK response sent. Your Supervisor has been notified.");buildNavigation();renderEmployeeOTAgreements();}catch(error){showToast(error?.message||"Unable to save your response.");}}));
  document.querySelectorAll("[data-agreement-no]").forEach(button=>button.addEventListener("click",()=>openEmployeeOTAgreementDeclineModal(button.dataset.agreementNo)));
  document.querySelectorAll("[data-agreement-file]").forEach(button=>button.addEventListener("click",()=>{const item=rows.find(row=>String(row.id)===String(button.dataset.agreementFile));if(!item)return;markEmployeeOTAgreementRead(item.id);employeeAttendanceDateKey=item.dutyDate;currentPage="attendance-ot";buildNavigation();renderEmployeeAttendanceOT(employee);}));
}
...
function renderEmployee(){
  ...
  if(currentPage==="advance-ot"){
    currentPage="attendance-ot"; // Compatibility for old bookmarks/session links.
    updateNavigationActiveState();
  }
  if(currentPage==="attendance-ot")return renderEmployeeAttendanceOT(employee);
  if(currentPage==="ot-agreements")return renderEmployeeOTAgreements();
  ...
  if(currentPage==="dashboard"){
    ...
    content.innerHTML=`
      <div class="ems-dashboard">
        ...
        <div class="ems-dashboard-main-grid">
          <div class="ems-dashboard-stack employee-dashboard-primary-stack">
            <div class="card">
            ...
            <div class="card"><div class="card-header"><div class="card-title-group"><h3>Quick Actions</h3><p>Employee self-service</p></div></div><div class="card-body"><div class="ems-quick-actions">
              ${emsDashboardQuickAction(todayIsDayOff?"File Rest Day OT":"Attendance & OT",todayIsDayOff?"Use after actual rest-day duty":"Choose today or a future OT date","clock","attendance-ot","blue")}
              ${emsDashboardQuickAction("OT Agreements","Answer Supervisor notices","check","ot-agreements","green")}
  ...
  if(currentPage==="file-leave"){
    ...
    document.getElementById("employeeLeaveRequestForm")?.addEventListener("submit",e=>{
      ...
      // Employee accounts do not receive the non-employee account directory.
      // Route by department instead; the reviewing Supervisor's real name is
      // recorded when they approve or reject this request.
      ...
      const request={
        ...
        position:employee.position,
        department:employee.department,
        supervisorName:supervisor?.displayName||`${employee.department} Supervisor`,
...
function renderSupervisorEmployeeSubmittedOT(){
    ...
    <div class="card supervisor-employee-submission-card">
      ...
      <div class="card-body">
        <div class="bulk-ot-toolbar employee-submission-toolbar">
          <label class="field compact-field"><span>Attendance / OT Duty Date</span><input id="employeeSubmissionOTDate" type="date" value="${escapeHtml(dateKey)}"><small>Defaults to today's date. Select another duty date when needed.</small></label>
                ...
                <div class="ot-entry-table-wrap"><table class="ot-entry-table employee-submission-table">
                  ...
                  <tbody>${shiftEmployees.length?shiftEmployees.map(emp=>{
                    ...
                    return `<tr class="employee-submission-row ${sub?"has-employee-submission":isAutoLeave?"automatic-leave-row":isAutoDayOff?"automatic-dayoff-row":isAutoAbsent?"automatic-absent-row":"waiting-employee-submission"}" data-employee-no="${escapeHtml(emp.no)}">
                      ...
                      <td class="ot-filed-time-cell">${employeeSubmissionHasOT(sub)?`<strong>${escapeHtml(formatDateTime(sub.submittedAt||sub.updatedAt||sub.createdAt))}</strong><small class="attendance-subline">Employee OT filing time</small>`:sub?`<span class="badge pending">Attendance only</span>`:""}</td>
  ...
  document.getElementById("employeeSubmissionOTDate")?.addEventListener("change",e=>{
    ...
    supervisorEmployeeSubmissionDateKey=/^\d{4}-\d{2}-\d{2}$/.test(selected)?selected:todayKey;
...
function renderSupervisor(){
  ...
  if(currentPage==="ot-agreements")return renderSupervisorOTAgreements();
...
const EMPLOYEE_MILESTONE_DEFINITIONS = [
  {key:"month3",months:3,label:"3-Month Evaluation",shortLabel:"3 months",purpose:"Employee evaluation",kind:"evaluation"},
  {key:"month5",months:5,label:"5-Month Evaluation",shortLabel:"5 months",purpose:"Employee evaluation",kind:"evaluation"},
  {key:"month12",months:12,label:"1-Year HMO Application",shortLabel:"1 year",purpose:"HMO application",kind:"hmo"}
];
...
function isEmployeeMilestoneCompleted(completions,definition,milestoneDate){
  const completion=completions?.[definition.key];
  if(!completion)return false;
  if(typeof completion==="string" || completion===true)return true;
  const completedMilestoneDate=String(completion.milestoneDate||"").trim();
  return !completedMilestoneDate || completedMilestoneDate===milestoneDate;
}
function getEmployeeMilestoneHistoryRows(profile={},referenceDate=new Date()){
  const dateHiredKey=String(profile?.employmentData?.dateHired||"").trim();
  if(!/^\d{4}-\d{2}-\d{2}$/.test(dateHiredKey))return [];
  const dateHired=parseLocalDate(dateHiredKey);
  if(Number.isNaN(dateHired.getTime()))return [];
  const completions=profile.milestoneCompletions||{};
  const today=new Date(referenceDate.getFullYear(),referenceDate.getMonth(),referenceDate.getDate());
  return EMPLOYEE_MILESTONE_DEFINITIONS.map(definition=>{
    const milestoneDate=addCalendarMonthsClamped(dateHired,definition.months);
    const milestoneDateKey=toDateKey(milestoneDate);
    const completion=completions?.[definition.key];
    const completed=isEmployeeMilestoneCompleted(completions,definition,milestoneDateKey);
    const daysUntil=calendarDayDifference(milestoneDate,today);
    return {
      milestone:definition.label,
      milestoneDate:milestoneDateKey,
      status:completed?"Completed":daysUntil<0?"Pending / Overdue":"Pending",
      completedAt:completed && typeof completion==="object"?String(completion.completedAt||""):"",
      completedBy:completed && typeof completion==="object"?String(completion.completedBy||""):""
    };
  });
}
function getEmployeeMilestoneNoticeData(referenceDate=new Date()){
  ...
  employees.forEach(employee=>{
    ...
    const completions=privateProfiles?.[employee.no]?.milestoneCompletions||{};
    const milestoneStages=EMPLOYEE_MILESTONE_DEFINITIONS.map(definition=>{
      const milestoneDate=addCalendarMonthsClamped(dateHired,definition.months);
      const daysUntil=calendarDayDifference(milestoneDate,today);
      const milestoneDateKey=toDateKey(milestoneDate);
      return {
        definition,milestoneDateKey,daysUntil,
        completed:isEmployeeMilestoneCompleted(completions,definition,milestoneDateKey)
      };
    });
    const activeStage=milestoneStages.find(stage=>!stage.completed && stage.daysUntil>=-EMPLOYEE_MILESTONE_REMINDER_AFTER_DAYS && stage.daysUntil<=EMPLOYEE_MILESTONE_REMINDER_BEFORE_DAYS)||null;
    if(!activeStage)return;
    const {definition,milestoneDateKey,daysUntil}=activeStage;
  ...
  const rows=notices.map(notice=>`
    <div class="employee-milestone-item ${notice.kind} ${notice.statusKey}">
      ...
      <div class="employee-milestone-main">
        ...
        <div class="employee-milestone-actions">
          <button type="button" class="btn btn-primary btn-sm employee-milestone-complete-btn" data-complete-milestone="${escapeHtml(notice.key)}" data-milestone-employee="${escapeHtml(notice.employeeNo)}" data-milestone-date="${escapeHtml(notice.milestoneDate)}" data-milestone-label="${escapeHtml(notice.label)}">OK / Done</button>
        </div>
      </div>
    </div>`).join("");
  return `
...
function bindEmployeeMilestoneNoticeActions(){
  ...
  document.querySelectorAll("[data-complete-milestone]").forEach(button=>button.addEventListener("click",async()=>{
    if(button.disabled || button.dataset.saving==="1")return;
    const employeeNo=String(button.dataset.milestoneEmployee||"").trim();
    const milestoneKey=String(button.dataset.completeMilestone||"").trim();
    const milestoneDate=String(button.dataset.milestoneDate||"").trim();
    const milestoneLabel=String(button.dataset.milestoneLabel||"Employee milestone").trim();
    const employee=getEmployeeByNo(employeeNo)||getHRMasterEmployeeRows().find(row=>String(row.no)===employeeNo);
    if(!employeeNo || !milestoneKey || !employee){showToast("Employee milestone could not be found.");return;}
    if(!window.confirm(`Mark ${milestoneLabel} for ${employee.name} as completed?`))return;
    button.dataset.saving="1";
    button.disabled=true;
    button.textContent="Saving…";
    const profiles=getEmployeePrivateProfiles();
    const profile=profiles[employeeNo]||{};
    const milestoneCompletions={...(profile.milestoneCompletions||{})};
    milestoneCompletions[milestoneKey]={
      completedAt:new Date().toISOString(),
      completedBy:currentUser.displayName||currentUser.username||currentUser.role||"Admin Supervisor",
      milestoneDate
    };
    profiles[employeeNo]={...profile,milestoneCompletions,updatedAt:new Date().toISOString()};
    try{
      const saved=await saveEmployeePrivateProfiles(profiles);
      const definitionIndex=EMPLOYEE_MILESTONE_DEFINITIONS.findIndex(definition=>definition.key===milestoneKey);
      const nextDefinition=definitionIndex>=0?EMPLOYEE_MILESTONE_DEFINITIONS[definitionIndex+1]:null;
      showToast(saved===false
        ? `${milestoneLabel} saved on this device. Firebase will retry syncing automatically.`
        : `${milestoneLabel} marked completed.${nextDefinition?` ${nextDefinition.label} will appear 1 week before its schedule.`:""}`);
      renderPage();
    }catch(error){
      console.error(error);
      showToast(error?.message||"Unable to complete this milestone.");
      button.disabled=false;
      button.textContent="OK / Done";
...
function renderHR(){
  ...
  if(currentPage==="employees"){
    ...
    const openEmployeeProfileView=async employeeNo=>{
      ...
      const milestoneHistory=getEmployeeMilestoneHistoryRows(profile,new Date());
        ...
        <div class="modal-backdrop employee-profile-modal-backdrop" id="employeeProfileViewBackdrop">
          <div class="modal employee-profile-view-modal" role="dialog" aria-modal="true" aria-labelledby="employeeProfileViewTitle">
            ...
            <div class="modal-body employee-profile-view-body" id="printableEmployeeProfile">
              ...
              ${employeeProfileArrayTable("Milestone & Evaluation History",milestoneHistory,[{label:"Milestone",value:"milestone"},{label:"Scheduled Date",value:row=>row.milestoneDate?formatDate(row.milestoneDate):""},{label:"Status",value:"status"},{label:"Completed On",value:row=>row.completedAt?formatDateTime(row.completedAt):""},{label:"Confirmed By",value:"completedBy"}])}
...
window.addEventListener("storage",e=>{
  ...
  if(e.key===OT_AGREEMENT_KEY && currentUser){
    updateEmployeeLeaveNotificationUI();
    updateSupervisorEmployeeNotificationUI();
    buildNavigation();
    if(currentUser.role==="Employee"){
      const latest=employeeOTAgreements(currentUser.employeeNo).sort((a,b)=>new Date(b.updatedAt||b.createdAt||0)-new Date(a.updatedAt||a.createdAt||0))[0];
      if(latest?.status==="Pending Response")showToast("New Day Off / Holiday overtime agreement received.");
      if(["dashboard","ot-agreements","attendance-ot"].includes(currentPage))renderPage();
    }else if(currentUser.role==="Supervisor"){
      const latest=departmentOTAgreements(currentUser.department).filter(row=>["Accepted","Declined"].includes(row.status)).sort((a,b)=>new Date(b.respondedAt||0)-new Date(a.respondedAt||0))[0];
      if(latest && !latest.supervisorResponseRead)showToast(`${latest.employeeName} answered ${latest.status==="Accepted"?"OK":"NO"} to the OT agreement.`);
      if(["dashboard","ot-agreements"].includes(currentPage))renderPage();
    }
  }
