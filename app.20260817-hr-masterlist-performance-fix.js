...
function buildNavigation(){
  ...
  const nav = {
    Supervisor:[
      ...
      ...(isAdminDepartmentSupervisor(currentUser)?[{id:"recruitment-monitoring",label:"Recruitment Monitoring",icon:"▤"}]:[{id:"my-requests",label:"Department Requests",icon:"▤"}]),
    ...
    content.innerHTML=`
      <div class="ems-dashboard">
        ...
        <div class="ems-dashboard-kpis">
          ...
          ${emsDashboardMetric(dashboardIsToday?"Today Requests":"OT Requests",dashboardRows.length,formatDate(dashboardDateKey),"blue","clipboard","my-requests")}
          ${emsDashboardMetric("Pending OT",pending+pendingReapprovals,"Waiting for Request Approver","orange","clock","my-requests")}
          ${emsDashboardMetric("Pending Leave",pendingLeaveRows.length,"Waiting for your review","purple","leave","leave-requests")}
          ${emsDashboardMetric(dashboardIsToday?"Today OT Hours":"OT Hours",dashboardHours.toFixed(2),`${formatDate(dashboardDateKey)} • recorded department OT hours`,"red","clock","my-requests")}
        </div>
...
function renderManager(){
  ...
  if(currentPage==="dashboard"){
    ...
    const departmentQueue=DEPARTMENTS.map(dep=>({
      label:dep,
      value:dashboardPendingOT.filter(r=>r.department===dep).length+dashboardPendingLeave.filter(r=>r.department===dep).length+pendingRecruitment.filter(r=>r.department===dep).length
    }));
    ...
    content.innerHTML=`
      <div class="ems-dashboard">
        ...
        <div class="ems-dashboard-main-grid">
          <div class="card">
            <div class="card-header"><div class="card-title-group"><h3>Department Approval Queue</h3><p>Pending OT + leave + recruitment by department</p></div><button type="button" class="ems-dashboard-link" data-dashboard-page="approvals">View queue</button></div>
            <div class="card-body">${emsDashboardBarChart(departmentQueue)}</div>
          </div>
          <div class="card">
