...
const OT_LOCATIONS = ["Bacao","Enlin","Yard 2.1"];
...
function getEmployeeLeaveCreditSummary(employeeNo,employmentStatusOverride=""){
...
function getMasterEmployeeRows(){
  return allKnownEmployees().map(emp=>{
    const detailed=getDepartmentEmployees(emp.department).find(row=>row.no===emp.no);
    const row=detailed || {...emp,location:"Unassigned",schedule:"Unassigned",dayOff:""};
    const employmentStatus=normalizeEmploymentStatus(row.employmentStatus,row.isCustom);
    const leaveCredits=getEmployeeLeaveCreditSummary(row.no,employmentStatus);
    return {
      ...row,
      employmentStatus,
      leaveCreditVL:leaveCredits.remainingVL,
      leaveCreditSL:leaveCredits.remainingSL,
      leaveUsedVL:leaveCredits.usedVL,
      leaveUsedSL:leaveCredits.usedSL,
      leaveReservedVL:leaveCredits.reservedVL,
      leaveReservedSL:leaveCredits.reservedSL
    };
...
function syncApprovedLeaveToDailyAttendance(leaveRequest){
  ...
  let count=0;
  eachDateKey(leaveRequest.startDate,leaveRequest.endDate).filter(date=>date<=todayKey).forEach(date=>{
...
function addManagerLeaveNotification(request){
  ...
  notifications.unshift({
    ...
    message:`${request.employeeName} • ${request.department} • ${request.leaveType} • ${request.payType||"Without Pay"}`,
    details:`HR approved • ${formatDate(request.startDate)}${request.endDate!==request.startDate?` – ${formatDate(request.endDate)}`:""}${isPaidLeaveRequest(request)?` • Deduct ${request.requestedCreditDays??leaveRequestDayCount(request.startDate,request.endDate)} credit day(s) after final approval`:""}`,
    leaveRequestId:request.id,
...
function renderPage(){
  ...
  const isAdminModulePage=ADMIN_MODULE_ACCESS.some(item=>item.id===currentPage);
...
function renderSupervisorLeaveRequests(){
  ...
  content.innerHTML=`
    ...
    <div class="stats-grid">
      ...
      ${statCard("Pending Request Approver",pendingApprover,"Already approved by HR")}
  ...
  return `<div class="card">
    ...
    <div class="table-wrap leave-table-wrap">
      <table class="leave-request-table employee-leave-history-table">
        <thead><tr><th>Request</th><th>Type</th><th>Leave Date</th><th>Reason</th><th>Supervisor</th><th>HR</th><th>Request Approver</th><th>Status</th></tr></thead>
        <tbody>${rows.length?rows.map(r=>`<tr>
          <td data-label="Request"><span class="request-id">${escapeHtml(r.id)}</span><br><small>${formatDateTime(r.createdAt)}</small><br><small class="leave-progress-label">${escapeHtml(leaveRequestProgressLabel(r))}</small></td>
        ...
        </tr>`).join(""):`<tr><td colspan="8">${emptyState("No leave requests yet","Use File Leave to submit your first VL / SL request.")}</td></tr>`}</tbody>
      </table>
    </div>
    ${paginationHtml}
  </div>`;
...
function renderLeaveApprovalPage(role){
  ...
  setPage(isHR?"Leave Approval — HR":"Leave Approval — Request Approver",isHR?"HR LEAVE REVIEW":"FINAL LEAVE APPROVAL",isHR?"Review employee-filed VL/SL requests already approved by the department Supervisor.":"Final approval for leave requests already approved by the Supervisor and HR.");
  content.innerHTML=`
    ${heroBanner(
      isHR?"HR leave approval queue":"Final leave approval queue",
      isHR?"HR is the second approval step after Supervisor review. Approved requests are automatically forwarded to the Request Approver.":"Request Approver gives the final approval. The final decision is automatically returned to the Supervisor by notification.",
      pending.length,
    ...
    <div class="stats-grid">
      ...
      ${statCard("Workflow",isHR?"Step 2 / 3 approvals":"Step 3 / 3 approvals",isHR?"After Supervisor approval":"Final Request Approver Review","primary")}
    </div>
    ${filterBar}
    <div class="card">
      <div class="card-header"><div class="card-title-group"><h3>${pending.length?"Pending Leave Requests":"Leave Request History"}</h3><p>${isHR?"Employee → Supervisor → HR → Request Approver":"Supervisor + HR approved requests requiring final Request Approver decision"}</p></div></div>
      <div class="table-wrap leave-table-wrap">
        <table class="leave-request-table">
          ...
          <tbody>${rows.length?rows.map(r=>{
            ...
            return `<tr class="${actionable?"leave-action-row":""}">
              ...
              <td class="wrap-cell">${escapeHtml(r.reason||"")}</td>
              <td>${r.supervisorReviewedAt?`<strong>${escapeHtml(r.supervisorReviewedBy||r.supervisorName||"Supervisor")}</strong><br><small>${formatDateTime(r.supervisorReviewedAt)}</small>${r.supervisorRemarks?`<br><small>${escapeHtml(r.supervisorRemarks)}</small>`:""}`:"—"}</td>
              <td>${leaveStatusBadge(r.status)}</td>
...
function renderManager(){
  ...
  if(currentPage==="dashboard"){
    ...
    const dashboardPendingLeave=pendingLeave.filter(r=>r.startDate<=dashboardDateKey && (r.endDate||r.startDate)>=dashboardDateKey);
    ...
    content.innerHTML=`
      ${heroBanner(
        ...
        `OT all filing dates + pending leave awaiting review`
      )}
      <div class="stats-grid approver-top-stats">
        ...
        ${statCard("Pending Leave",dashboardPendingLeave.length,`Leave effective today • ${formatDate(dashboardDateKey)}`,"warning","leave-approvals")}
...
function renderHR(){
  ...
  if(currentPage==="employees"){
    ...
    const masterAttendanceRecords=getApprovedAttendanceRecords();
    const attendanceStatsByEmployee=new Map(masterEmployees.map(emp=>{
      const employeeNo=String(emp.no||"").trim().toLowerCase();
      const employeeName=String(emp.name||"").trim().toLowerCase();
      ...
      masterAttendanceRecords.forEach(record=>{
        ...
        if(!((employeeNo&&recordNo===employeeNo)||(!recordNo&&employeeName&&recordName===employeeName)))return;
      ...
      const records=[...unique.values()].sort((a,b)=>String(b.date).localeCompare(String(a.date)));
      return [emp.no,{
        leaveRecords:records.filter(record=>record.status==="Leave"),
        absentRecords:records.filter(record=>record.status==="Absent")
      }];
    }));
...
function preload(){
  // Run once for this build even when the browser already contains data from an older ZIP.
  if(localStorage.getItem(FULL_FLOW_RESET_REVISION_KEY)!==FULL_FLOW_RESET_REVISION){
    clearAllDataKeepAccountsForFlowTest();
  }

  if(localStorage.getItem(STORE_KEY)===null)saveRequests([]);
  if(localStorage.getItem(LEAVE_REQUEST_KEY)===null)saveLeaveRequests([]);
  if(localStorage.getItem(ATTENDANCE_STORE_KEY)===null)saveDailyAttendanceRecords([]);
  if(localStorage.getItem(EMPLOYEE_ATTENDANCE_OT_KEY)===null)saveEmployeeAttendanceOTSubmissions([]);
}
